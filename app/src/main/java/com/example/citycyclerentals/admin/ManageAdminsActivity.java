...
public class ManageAdminsActivity extends AppCompatActivity implements UserAdapter.OnItemClickListener {

    private static final String TAG = "ManageUsersActivity";
    private RecyclerView recyclerView;
    private UserAdapter adapter;
    private List<com.example.citycyclerentals.admin.User> userList;
    ...
    private void fetchUsers() {
        new Thread(new Runnable() {
            ...
            public void run() {
                try {
                    URL url = new URL("http://192.168.1.2/CityCycle%20Rentals/get_admins.php"); // Update with your server URL
                    ...
                    for (int i = 0; i < users.length(); i++) {
                        ...
                        userList.add(new com.example.citycyclerentals.admin.User(id, username, email, phoneNumber, role, profilePicture));
                    }
    ...
    private void addUser(final String username, final String email, final String phoneNumber, final String password) {
        new Thread(new Runnable() {
            ...
            public void run() {
                try {
                    URL url = new URL("http://192.168.1.2/CityCycle%20Rentals/add_admin.php");
                    HttpURLConnection conn = (HttpURLConnection) url.openConnection();
                    ...
                    runOnUiThread(new Runnable() {
                        @Override
                        public void run() {
                            Toast.makeText(ManageAdminsActivity.this, response, Toast.LENGTH_LONG).show();
    ...
    private void showEditUserDialog(final int position) {
        ...
        final com.example.citycyclerentals.admin.User user = userList.get(position);
        ...
        uploadImageButton.setOnClickListener(new View.OnClickListener() {
            @Override
            public void onClick(View v) {
                ImagePicker.with(ManageAdminsActivity.this)
            ...
            public void run() {
                try {
                    ...
                    runOnUiThread(new Runnable() {
                        ...
                        public void run() {
                            ...
                            if (response.equals("Record updated successfully")) {
                                userList.set(position, new com.example.citycyclerentals.admin.User(userId, username, email, phoneNumber, userList.get(position).getRole(), profilePicture));
                                adapter.notifyDataSetChanged();
                            } else {
                                Log.e(TAG, "Error updating admin: " + response);
                                Toast.makeText(ManageAdminsActivity.this, response, Toast.LENGTH_LONG).show();
                            }
                        }
                    });
                    ...
                    runOnUiThread(new Runnable() {
                        @Override
                        public void run() {
                            Toast.makeText(ManageAdminsActivity.this, "Error: " + e.getMessage(), Toast.LENGTH_LONG).show();
                        }
                    });
                }
            }
    ...
    private void uploadImage(Uri fileUri) {
        ...
        client.newCall(request).enqueue(new Callback() {
            ...
            public void onFailure(Call call, IOException e) {
                ...
                new Handler(Looper.getMainLooper()).post(() -> Toast.makeText(ManageAdminsActivity.this, "Image upload failed", Toast.LENGTH_SHORT).show());
            }
                    ...
                    try {
                        ...
                        if (jsonObject.has("error")) {
                            new Handler(Looper.getMainLooper()).post(() -> {
                                try {
                                    Toast.makeText(ManageAdminsActivity.this, jsonObject.getString("error"), Toast.LENGTH_SHORT).show();
                            ...
                            new Handler(Looper.getMainLooper()).post(() -> Toast.makeText(ManageAdminsActivity.this, "Image uploaded successfully", Toast.LENGTH_SHORT).show());
                        }
                    } catch (JSONException e) {
                        Log.e(TAG, "Failed to parse upload response", e);
                        new Handler(Looper.getMainLooper()).post(() -> Toast.makeText(ManageAdminsActivity.this, "Failed to parse upload response", Toast.LENGTH_SHORT).show());
                    }
                }
            }
        });
